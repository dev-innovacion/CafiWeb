...
namespace RivkaAreas.ObjectAdmin.Models
{
    public class Demand : MongoModel
    {
        ...
        public string GetDemandTable(string varMovement, string varStatus, string Creator = null,bool admin=false)
        {
            ...
            statusArray.Add("3", new BsonArray(new int[] { 3}));
            ...
            statusArray.Add("10", new BsonArray(new int[] { 5 }));
            ...
            IMongoQuery query;
            IMongoQuery istemporal = null;
            bool flag = false;
            if (varStatus == "5")
            {
                istemporal = Query.And(Query.EQ("temporal", "False"));
                flag = true;
            }
            else if (varStatus == "10")
            {
                istemporal = Query.And(Query.EQ("temporal", "True"));
                flag = true;
            }
            if (varMovement == "null")
            {
            ...
            if (Creator != null && admin==false)
            {
                ...
                if (flag)
                {
                    mainQuery = Query.And(singleQuery, query, istemporal);
                }
                else
                {
            ...
            else
            {
                if (flag)
                {
                    mainQuery = Query.And(istemporal, query);
                }
                else
                {
                    mainQuery = query;
