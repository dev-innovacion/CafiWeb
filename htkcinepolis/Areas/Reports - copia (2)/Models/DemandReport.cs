               ...
               // .Join("MovementProfiles", "movement", "_id", "name=>nameMovement,typeMovement=>typemov");
                ...
                if (movement != null && movement.Count > 0)
                {
                    if (start == end)
                    {
                        ...
                        {
                            query = Query.And(Query.In("movement", bsonarraymov), Query.In("Creator", bsonarrayusers), Query.In("objects.location", bsonarraylocs), Query.In("_id", bsonarrayobjs), Query.In("status", bsonarraystatus), Query.GTE("CreatedTimeStamp", start), Query.LTE("CreatedTimeStamp", end + 1000000));
                        }
                        else
                        {
                            query = Query.And(Query.In("movement", bsonarraymov), Query.In("Creator", bsonarrayusers), Query.In("objects.location", bsonarraylocs), Query.In("status", bsonarraystatus), Query.GTE("CreatedTimeStamp", start), Query.LTE("CreatedTimeStamp", end + 1000000));
