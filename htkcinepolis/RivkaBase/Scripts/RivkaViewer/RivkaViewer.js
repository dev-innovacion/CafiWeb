...
function RivkaViewer(containerID) {
    ...
    var table = {
        ...
        print: function () {
            ...
            var fragment1 = jQuery(document.createDocumentFragment());
            for (i = 0; i < data.length; i++) {
                var newTr = table.createNode(data[i]);
                jQuery(fragment1).append(newTr)
               
            }
            debugger;
            tableElement.find("tbody").append(fragment1);
        ...
        createNode: function (obj) {
            ...
            for (key in actionKeys) {
                try{
                    var icon="";
                    if (extraAction[actionKeys[key]] == undefined) {
                        if(count==0)
                            icon = "icon-edit"
                        else
                            icon = "icon-trash"
                        ...
                        try{
                            icon = extraAction[actionKeys[key]].icon;
                        }catch(ex){ }
                    }
            ...
            jQuery(fragment).html(jQuery(newTr));
