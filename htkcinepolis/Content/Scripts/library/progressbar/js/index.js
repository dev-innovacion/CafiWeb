/* SET RANDOM LOADER COLORS FOR DEMO PURPOSES */	
	var demoColorArray = ['red','blue1','green1','yellow','purple','gray1'];
	...
	$(window).load(function($) {
		...
		$progress.animate({
			...
			jQuery('#progress span').text('Loading...!').addClass('loaded');
			$percent.text('0%');
	...
	function preload(imgArray) {
		...
		calcPercent = setInterval(function() {
			...
			$percent.text(Math.floor(($progress.width() / jQuery('.loader').width()) * 100) + '%');
	...
	function setSkin(skin){
		jQuery('#progress .loader').attr('class', 'loader '+skin);
